...
| 段 | 鳴らすもの |
| --- | --- |
| 大津波警報 | `tsunami_alarm` (nc246850.mp3) を **7 秒**で切って鳴らし、続けて `tsunami_major` (tsa04.mp3) |
| 津波警報 | 同上、続きが `tsunami_warning` (tsa03.mp3) |
| 津波注意報 | `tsunami_advisory` (tsa02.mp3)。無いときは `tsunami_forecast` で代用 |
| 津波予報 | `tsunami_forecast` (tsa01.mp3) だけ |

読み上げは効果音がすべて鳴り終わってから始まります。
...
地震情報などの読み上げは Scratch の音声合成 (ネズミ ×1.19) で作っている。
緊急地震速報 (警報) の読み上げだけは VOICEVOX の四国めたん (ノーマル) で、
`tools/generate_metan_voice.py` で作る。

    VOICEVOX:四国めたん

利用にあたってはこのクレジットが要る (画面の地震情報パネルの下に出している)。
音源そのものはここには置かない。
